...
class SimpleVectorStore:
	...
	def search(self, query: str, top_k: int = 5):
		"""Return list of (text, metadata, score) sorted by similarity."""
		...
		for score, idx in zip(scores, indices):
			idx_int = int(idx)
			if idx_int != -1:
				results.append((self.texts[idx_int], self.metadatas[idx_int], float(score)))
		return results


def index_exists(persist_path: str) -> bool:
	return (
		os.path.isdir(persist_path)
		and Path(os.path.join(persist_path, "index.faiss")).exists()
		and Path(os.path.join(persist_path, "meta.pkl")).exists()
	)
